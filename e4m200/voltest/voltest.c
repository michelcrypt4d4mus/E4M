...
main (int argc, char *argv[])
{
  ...
  memset (sd, 0, 160);
  memcpy (sd, szPassword, strlen (szPassword));
  shash ((char *) sd, 160, &S, digestptr);

  x = fread (buffer, 1, sizeof (buffer), f);
  if (x != sizeof (buffer))
    {
      printf ("This volume is too small. The volume must be %d kb in size.", sizeof (buffer) / 1024);
      fclose (f);
      exit (1);
    }

  x = SD_ReadHeader (buffer, &nVolType, digestptr, &cryptoInfo);
  if (x == 0)
    {
      char tmp[512];
      ...
      if (sector_flag != 0)
	{
      ...
      printf ("Success, scramdisk volume cipher = %s\n", get_cipher_name (cryptoInfo->cipher));
    ...
    {
      printf ("Failure, not a scramdisk volume or wrong password\n");
    }
