...
E4MSendIRP_FileDevice (PDEVICE_OBJECT DeviceObject,
		       ...
		       PIRP Irp)
{
  ...
  if (NewIrp != NULL)
    {
      ...
      /* Doc's say to copy the Tail.Overlay.Thread from the original Irp but
         if I do this the user is not prompted on media removed and other
         user conditions! */
    ...
    {
      e4mfree (pUserBuffer);	/* Free the temp buffer we allocated for the
				   IRP */
  ...
  IoSetCompletionRoutine (NewIrp, E4MCompletion, Irp, TRUE, TRUE, TRUE);
