...
#define FIRST_READ_SIZE SECTOR_SIZE * 21
	       ...
	       BOOL bRawDevice)
{
  ...
  if (ntStatus == 0xc000001b) 
  {
	/* Partitions which return this code can still be opened with FILE_SHARE_READ but this causes
	NT problems elsewhere in particular if you do FILE_SHARE_READ NT will die later if anyone
	even tries to open the partition  (or file for that matter...)  */
  ...
  if (lDiskLength.QuadPart < MIN_VOLUME_SIZE || lDiskLength.QuadPart > MAX_VOLUME_SIZE)
    {
      ...
      mount->nReturnCode = ERR_VOLUME_SIZE_WRONG;
      ntStatus = STATUS_SUCCESS;
      goto error;
    }
  else
  ...
  /* Save away the real boot sector, because if this turns out to be an SFS
     volume then we need this information later, if this is a E4M volume
     the info is not needed */
  ...
  if (mount->bSD == FALSE)
	mount->nReturnCode = VolumeReadHeader (readBuffer, &Extension->nVolType, mount->szPassword, &Extension->cryptoInfo);
  else
	mount->nReturnCode = SD_ReadHeader (readBuffer, &Extension->nVolType, mount->szPassword,&Extension->cryptoInfo);
  ...
  if (mount->nReturnCode  == ERR_PASSWORD_WRONG)
  {
	  ...
	  for ( i = 0 ; i < 4 ; i ++ ) 
	  {
		...
		{
			if (mount->bSD == FALSE) 
				mount->nReturnCode = VolumeReadHeader (readBuffer, &Extension->nVolType, 
							szDriverPassword[i],&Extension->cryptoInfo);
			else
				mount->nReturnCode = SD_ReadHeader (readBuffer, &Extension->nVolType,
						szDriverPassword[i],  &Extension->cryptoInfo);
  ...
  if (mount->bCache == TRUE && mount->nPasswordLen > 0) 
  {
	memcpy(szDriverPassword[nPasswordIdx], mount->szPassword,mount->nPasswordLen);
	nDriverPasswordLen[nPasswordIdx] = mount->nPasswordLen;
	nPasswordIdx = (nPasswordIdx + 1) % 4;
  }
  ...
  if (mount->nReturnCode == 0)
      ...
      else if (isE4M(Extension->nVolType)==TRUE)
	{
	  ...
	  Extension->cryptoInfo->decrypt_sector ((unsigned long *) boot_sector, 1,1,
      ...
      else if (Extension->nVolType == SD_VOLTYPE)
        {
	  /* Handle the volume setup for ScramDisk */

	  boot_sector = (struct msdos_boot_sector *) (readBuffer + 8192 + 2048);

	  /* There's 20 sectors more than there should be */	
	  Extension->DiskLength -= SECTOR_SIZE * 20;
	 
	  /* It's in the volume file so we must decrypt it */
	  Extension->cryptoInfo->decrypt_sector ((unsigned long *) boot_sector, 4,1,
					      &Extension->cryptoInfo->ks[0],
					Extension->cryptoInfo->iv,
					Extension->cryptoInfo->cipher);
	...
	Extension->SectorsPerTrack / Extension->TracksPerCylinder;
      ...
      if (wcslen(pwszMountVolume) < sizeof(Extension->wszVolume)) 
	wcscpy(Extension->wszVolume,pwszMountVolume);
      else 
      {
	memcpy (Extension->wszVolume, pwszMountVolume, sizeof(Extension->wszVolume));
	Extension->wszVolume[sizeof(Extension->wszVolume)-1] = (WCHAR) '.';
      }
  ...
  /* Failed due to some non-OS reason so we drop through and return NT SUCCESS
     then nReturnCode is checked later in user-mode */
...
error:
  ...
  /* The cryptoInfo pointer is deallocated if the readheader routines fail so there
     is no need to deallocate here  */
...
E4MCompletion (PDEVICE_OBJECT DeviceObject, PIRP Irp, PVOID pUserBuffer)
{
  ...
  /* Note: The Irp stack location we get back here is our one, we setup the
     next stack location with a copy of this stack data in the send
     function... here we always get back our own stack location, so it's
     possible to use Read.Key to store extra pointers if needed. */
  ...
  if (Irp->PendingReturned)	/* From Windows NT File System Internals */
    IoMarkIrpPending (Irp);

  if (Extension->bRawDevice == FALSE)
    {
      /* Note: For some reason even though we used DIRECT_IO sometimes the
         Irp's come back to use with MDLs !! if we get an MDL here we need to
         free it up otherwise later when we call IoFreeIrp the system will
         trap */
  ...
  if (NT_SUCCESS (Irp->IoStatus.Status) && irpSp->MajorFunction == IRP_MJ_READ)
      ...
      if (tmpLength > 0)
	{
	  ...
	  if (Extension->nVolType == SD_VOLTYPE)
		secNum = tmpOffset / SECTOR_SIZE + 4 - 20;
	  else
		secNum = tmpOffset / SECTOR_SIZE;

	  /* Decrypt the data on read */
	  Extension->cryptoInfo->decrypt_sector ((unsigned long *) CurrentAddress,
						    secNum,
						    tmpLength / SECTOR_SIZE,
					      &Extension->cryptoInfo->ks[0],
					Extension->cryptoInfo->iv,
  ...
  if (NT_SUCCESS (Irp->IoStatus.Status) && irpSp->MajorFunction == IRP_MJ_WRITE)
      ...
      if (NT_SUCCESS (Irp->IoStatus.Status))
	{
	  ...
	  if (Extension->nVolType == SFS_VOLTYPE && tmpOffset == 0)
	    {
	      /* Copy the boot sector Windows NT tried to write to begin with
	         into the fake boot sector area, next time Windows NT reads
	         the boot sector it will actually get the fake area updated
	         with whatever it tried to write here! */
  ...
  if (Extension->bRawDevice == FALSE)
    {
      ...
      /* Free the allocated IRP. Note: This must be done before we free
         tmpBuffer! */
      IoFreeIrp (Irp);
...
E4MReadWrite (PDEVICE_OBJECT DeviceObject, PEXTENSION Extension, PIRP Irp)
{
  ...
  /* Check for invalid parameters.  It is an error for the starting offset +
     length to go past the end of the buffer, or for the length to not be a
     proper multiple of the sector size. Others are possible, but we don't
     check them since we trust the file system and they aren't deadly.  */
  ...
  if (irpSp->MajorFunction == IRP_MJ_READ)
	...
	irpSp->Parameters.Read.ByteOffset.LowPart += SECTOR_SIZE;
      ...
      if (Extension->nVolType == SD_VOLTYPE)
	irpSp->Parameters.Read.ByteOffset.LowPart += SECTOR_SIZE * 20;
      ...
      else
	{
	  ...
	  if (Extension->nVolType == SD_VOLTYPE)
		secNum = irpSp->Parameters.Read.ByteOffset.LowPart / SECTOR_SIZE + 4 - 20;
	  else
