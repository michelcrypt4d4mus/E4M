...
GetArgumentID (char *lpszArgument, int *nArgPos)
{
  argument args[]=
  {
    ...
    {"/password2", "/p2", PASSWORD_ARG2},
    {"/password3", "/p3", PASSWORD_ARG3},
    {"/password4", "/p4", PASSWORD_ARG4},
    {"/auto", "/a", AUTO_ARG},
    {"/cache", "/c", CACHE_ARG},
		  ...
		  int nNoCommandLineArgs, char *lpszValue, int nValueSize)
{
  *lpszValue = 0;
  ...
  if (nArgPos)
    {
      /* Handles the case of no space between parameter code and value */
  ...
  else if (*nArgIdx + 1 != nNoCommandLineArgs)
    {
      ...
      if (x == 0)
	{
	  /* Handles the case of space between parameter code and value */
