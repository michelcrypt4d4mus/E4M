...
static void bcopy_u4_byte(unsigned long k, unsigned char * b) { 
  b[0]=(unsigned char) (k>>24)&0xFF;
  b[1]=(unsigned char)(k>>16)&0xFF;
...
void misty1_keyinit(unsigned long  *ek, unsigned long  *k)
{
  ...
  bcopy_u4_byte(k[0],(unsigned char *)(&key[0]));
  bcopy_u4_byte(k[1],(unsigned char *)(&key[4]));
  bcopy_u4_byte(k[2],(unsigned char *)(&key[8]));
  bcopy_u4_byte(k[3],(unsigned char *)(&key[12]));
  
  memset (ek,0,MISTY1_KEYSIZE*sizeof(unsigned long) );
...
void misty1_key_destroy(unsigned long  *ek)
{
	memset (ek,0,MISTY1_KEYSIZE*sizeof(unsigned long) );
}
...
*/
   ...
   unsigned long  Plaintext[]= {0x01234567, 0x89abcdef, 0xfedcba98, 0x76543210};
   unsigned long  Ciphertext[]= { 0x8b1da5f5, 0x6ab3d07c, 0x04b68240, 0xb13be95d};
   unsigned long  ek_e[MISTY1_KEYSIZE],  ek_d[MISTY1_KEYSIZE];
   unsigned long  c[4];
   ...
   if (!memcmp(c,Ciphertext,4 * sizeof(unsigned long))) {
   ...
   if (memcmp(ek_e,ek_d,MISTY1_KEYSIZE*sizeof(unsigned long))) {
   ...
   if (!memcmp(c,Plaintext,4 * sizeof(unsigned long))) {
