...
extern BOOL UpdateProgressBar (int nSecNo);
	     ...
	     int nFrequency)
{
  ...
  if (*write_buf_cnt == WRITE_BUF_SIZE)
    {
      if (_lwrite (dev, write_buf, WRITE_BUF_SIZE) == HFILE_ERROR)
	return FALSE;
      else
	*write_buf_cnt = 0;
    }
...
int
Format (fatparams * ft, HFILE dev, int nVolType, PCRYPTO_INFO cryptoInfo, int nFrequency)
{
  int write_buf_cnt = 0;
  ...
  if (_llseek (dev, 0, FILE_BEGIN) == HFILE_ERROR)
    return FALSE;
  else if (_lwrite (dev, sector, SECTOR_SIZE) == HFILE_ERROR)
    return FALSE;
  else if (_llseek (dev, 0, FILE_BEGIN) == HFILE_ERROR)
    return FALSE;
  else if (_lwrite (dev, (char *) &ft->header, SECTOR_SIZE) == HFILE_ERROR)
    return FALSE;
  ...
  if (write_buf_cnt != 0 && _lwrite (dev, write_buf, write_buf_cnt) == HFILE_ERROR)
    goto fail;
