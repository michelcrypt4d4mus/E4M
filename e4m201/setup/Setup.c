...
char work_dir[E4M_MAX_PATH];
BOOL bUninstall = FALSE;
BOOL bDone = FALSE;

UINT APIENTRY
OFNHookProcOldStyle (HWND hdlg, UINT uiMsg, WPARAM wParam, LPARAM lParam)
{
	if (lParam);		/* Remove warning */

	if (uiMsg == WM_COMMAND && wParam == IDOK)
	{
		GetWindowText (GetDlgItem (hdlg, 1088), dlg_file_name, sizeof (dlg_file_name));
...
BOOL
BrowseFiles2 (HWND hwndDlg, LPCSTR lpszTitle, LPSTR lpszFileName)
{
	OPENFILENAME ofn;
	...
	*szFileTitle = *lpszFileName = 0;
	...
	ofn.lpstrDefExt = "*.*";
	ofn.hInstance = GET_INSTANCE (hwndDlg);
	ofn.lpTemplateName = MAKEINTRESOURCE (IDD_OPENTEMP);
	...
	*dlg_file_name = 0;
	...
	if (!GetOpenFileName (&ofn))
	{
		if (*dlg_file_name != 0)
		{
			strcpy (lpszFileName, dlg_file_name);
			return TRUE;
		}
...
InstallDlgProc (HWND hwndDlg, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COMMAND:
		...
		{
			char szDirname[E4M_MAX_PATH];
			if (BrowseFiles2 (hwndDlg, "Select Directory", szDirname) == TRUE)
