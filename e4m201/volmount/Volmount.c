...
SaveSettings (HWND hwndDlg)
{
	...
	if (bHistory == FALSE)
	{
		/* User wants no settings, stop now, and remove old settings
		   if found */
		remove ("e4m.ini");
		return;
	}
...
#if 0
					/* Currently there are no mount
					   warnings defined */
					if (bNoMountWarning != TRUE)
					{
						int x = DialogBoxParam (GET_INSTANCE (hwndDlg), MAKEINTRESOURCE (IDD_WARNING_DLG),
									hwndDlg, (DLGPROC) WarningDlgProc, (LPARAM) NULL);
