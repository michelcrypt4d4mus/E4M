...
#define IDEA_TEST_COUNT 11

IDEA_TEST idea_ecb_vectors[IDEA_TEST_COUNT] = {
...
CipherTestDialogProc (HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COMMAND:
		...
		if (lw == IDC_AUTO)
		{
			...
			for (i=0;i<IDEA_TEST_COUNT;i++)
			{
				int cipher = IDEA;
				memcpy(key, idea_ecb_vectors[i].key, 16);
				memcpy(tmp, idea_ecb_vectors[i].plaintext, 8);
				init_cipher2(cipher, key, ks_tmp, 16);
				encipher_block(cipher, tmp, ks_tmp);
				if (memcmp(idea_ecb_vectors[i].ciphertext, tmp,8)!=0)
					break;
			}
	
			if (i!=IDEA_TEST_COUNT)
				bFailed = TRUE;
